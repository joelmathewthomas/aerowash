...
public class FlatDeleteServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public FlatDeleteServlet() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try (Connection conn = DriverManager.getConnection(getServletContext().getInitParameter("DbUrl"),
			...
			try {
				flat_id = Integer.parseInt(request.getParameter("fid"));
				customer_id = Integer.parseInt(request.getParameter("cid"));
			} catch (NumberFormatException ex) {
				ex.printStackTrace();
				response.sendRedirect("status?c=4&r=5&e=invalid_flat_id_or_customer_id");
				return;
			}
			
			String error = Flat.deleteRecord(conn, flat_id);

			if (error == null) {
				response.sendRedirect("flat?cid=" + customer_id);
			} else {
